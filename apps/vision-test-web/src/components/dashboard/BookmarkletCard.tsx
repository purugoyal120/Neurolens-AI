...
export const BookmarkletCard: React.FC = () => {
  ...
  return (
    <div className={`rounded-3xl p-6 border shadow-xl shadow-slate-200/50 relative overflow-hidden group h-full flex flex-col transition-colors duration-500 ${isActivated ? 'bg-blue-50 border-blue-200' : 'bg-white border-slate-100'}`}>
      <div className={`absolute top-0 right-0 w-32 h-32 rounded-bl-full -z-10 transition-transform duration-700 ${isActivated ? 'bg-blue-500/10 scale-150' : 'bg-blue-500/5 group-hover:scale-110'}`}></div>
      
      <div className="flex items-start justify-between mb-6">
        <div>
          <div className="flex items-center gap-2 mb-1">
            <Power className={`w-5 h-5 ${isActivated ? 'text-blue-600' : 'text-slate-400'}`} />
      ...
      <div className={`rounded-2xl p-4 mb-6 border flex items-center justify-between transition-colors ${isActivated ? 'bg-blue-100/50 border-blue-200' : 'bg-slate-50 border-slate-100'}`}>
        <div>
          <div className="text-xs font-bold text-slate-400 uppercase tracking-wider mb-1">Diagnostic Profile</div>
          <div className={`font-bold px-3 py-1.5 rounded-xl border inline-block ${isActivated ? 'text-blue-700 bg-white border-blue-300 shadow-sm' : 'text-slate-600 bg-white border-slate-200'}`}>
            {profileDisplay}
          </div>
        </div>
        <div className="h-8 w-[1px] bg-slate-200 mx-2 hidden sm:block"></div>
        <div className="text-right flex items-center gap-2 hidden sm:flex">
          <ShieldCheck className={`w-4 h-4 ${isActivated ? 'text-blue-500' : 'text-slate-400'}`} />
        ...
        <div className="text-center animate-in fade-in duration-300">
          ...
          <button
            className={`inline-flex items-center gap-2 px-8 py-4 rounded-full font-extrabold text-lg shadow-xl transition-all duration-300 border-2 border-white ${
              isActivated 
                ? 'bg-blue-100 text-blue-700 hover:bg-blue-200 shadow-blue-500/10' 
                : 'bg-blue-500 text-white hover:scale-105 hover:bg-blue-400 shadow-blue-500/30'
            }`}
      ...
      <div className="mt-6 pt-4 border-t border-slate-100 flex justify-between items-center text-xs">
        ...
        <span className={`font-bold ${isActivated ? 'text-blue-600' : 'text-slate-400'}`}>
